...
(Packet . ≜ . (U Integer (Listof Packet)))

(define input : (Listof Packet)
...
(define cmp : (Packet Packet → Ord)
  (match-lambda**
   [((? integer? n₁) (? integer? n₂))
    (cond [(< n₁ n₂) '<]
          [(= n₁ n₂) '=]
          [else '>])]
   [((? list? l₁) (? integer? n₂)) (cmp l₁ (list n₂))]
   [((? integer? n₁) (? list? l₂)) (cmp (list n₁) l₂)]
   [((cons p₁ l₁) (cons p₂ l₂))
    (match (cmp p₁ p₂)
      ['= (cmp l₁ l₂)]
      ['< '<]
...
(: in-order-indices : (Listof Packet) → (Listof Integer))
(define (in-order-indices ps)
  (let loop : (Listof Integer) ([ps : (Listof Packet) ps]
                                [i : Integer 1])
    (match ps
      [(list* p₁ p₂ ps*)
       (match (cmp p₁ p₂)
         [(or '< '=) (cons i (loop ps* (add1 i)))]
         [_ (loop ps* (add1 i))])]
      ['() '()])))

(define (q1)
  (apply + (in-order-indices input)))
...
(define (q2)
  (define sorted
    (sort
     (list* '((2)) '((6)) input)
     (λ ([p₁ : Packet] [p₂ : Packet])
       (and (member (cmp p₁ p₂) '(< =)) #t))))
  (define i₁ (add1 (assert (index-of sorted '((2))))))
  (define i₂ (add1 (assert (index-of sorted '((6))))))
  (* i₁ i₂))
